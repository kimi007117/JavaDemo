...
public class FaceRectView extends SurfaceView implements SurfaceHolder.Callback {
     ...
     * 绘制人脸识别框
     */
    public void drawFaceRect(DetectorData mDetectorData) {
        Canvas canvas = mHolder.lockCanvas();
        if (null == canvas) {
            return;
        }

        if (mDetectorData == null || mDetectorData.faceRectList == null) {
            mHolder.unlockCanvasAndPost(canvas);
            return;
        }

        canvas.drawColor(0, PorterDuff.Mode.CLEAR);
        if (mDetectorData.faceRectList != null && mDetectorData.faceRectList.length > 0) {
            for (Rect rect : mDetectorData.faceRectList) {
