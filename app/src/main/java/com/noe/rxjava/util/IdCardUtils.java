 ...
 * 当今的身份证号码只有18位。1985年我国实行居民身份证制度，当时签发的身份证号码是15位的，2012年12月31日15位身份证正式废止，不再使用。1999年签发的身份证由于年份的扩展（由两位变为四位）和末尾加了效验码，就成了18位。
 * （1）前1、2位数字表示：所在省份的代码；
 * （2）第3、4位数字表示：所在城市的代码；
 * （3）第5、6位数字表示：所在区县的代码；
 ...
 * <p>
 * Created by lijie on 2018/12/13.
 */
public class IdCardUtils {
    ...
    public static boolean isVerifyCode(String str) {
        ...
        if (TextUtils.isEmpty(areaCode.get(ai.substring(0, 2)))) {
            return false;
        }

        return isVerifyCode(ai, str);
    }
    ...
    /**
     * 身份证简单校验，只校验18位，并且最后一位可以是数字或者x或者X
     *
     * @param str 身份证号
     * @return 返回结果
     */
    public static boolean simpleVerifyCode(String str) {
        if (TextUtils.isEmpty(str) || str.length() != 18) {
            return false;
        }
